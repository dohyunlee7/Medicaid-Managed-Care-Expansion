...
# Categorize states into treatment groups, just looking at reverter states
treated_states <- c("connecticut", "oklahoma", "vermont")
control_states <- setdiff(unique(main_data$state), treated_states)


# Create variable for treatment categories
# re-define treatment year
# Standard: Year when state reverted back to FFS
main_data <- main_data %>%
  mutate(category = ifelse(state %in% treated_states, "treated", "control")) %>%
...
dat <- main_data %>%
  filter(state %in% c(treated_states, control_states)) %>%
  mutate(center_time = ifelse(state %in% treated_states, 
                              year - treatment_year, 
                              0)) %>%
  dummy_cols(select_columns = "center_time") %>%
  mutate(treated = case_when(
    state == "connecticut" & year >= 2012 ~ 1,
    state == "oklahoma" & year >= 2004 ~ 1,
    state == "vermont" & year >= 2001 ~ 1,
    
    state %in% treated_states & year >= treatment_year ~ 1,
    state %in% treated_states & year < treatment_year ~ 0,
    state %in% control_states ~ 0,
    TRUE ~ 0
  ))
...
dat <- dat %>%
  filter(state %in% c("connecticut", "oklahoma", "vermont",
         ...
         "florida", "hawaii", "illinois", "indiana", "kansas", "maryland",
...
did_graph_10 <- function(outcome, outcome_name){
  formula_str1 <- paste0(
    outcome, " ~ ",
    "`center_time_-10` + `center_time_-9` + `center_time_-8` + `center_time_-7` + `center_time_-6` + `center_time_-5` + ",
