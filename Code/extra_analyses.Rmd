...
packages <- c("ggplot2", "estimatr", "readxl", "readr", "dplyr",
              ...
              "synthdid", "pubtheme", "gridExtra", "tinytex", "did")
...
did_graph_trend_10 <- function(outcome, outcome_name, cohort_interval, cohort_var){
  p <- list()
  
  cohort_interval <- sym(cohort_interval)
  
  cohort_data <- data_cohorts %>%
    filter(!!cohort_interval == cohort_var | !!cohort_interval == "never-treated")
  ...
  p1 <- ggplot(data = es_d, aes(x = t, y = estimate, group = 1))+
    ...
    labs(x = "Relative Time", y = "Estimate",
         title = paste0(outcome_name, " - ", cohort_var))+
...
for (outcome_name in names(outcomes)) {
  ...
  for (cohort in cohorts) {
    # Generate the plot
    p <- did_graph_trend_10(outcome_name, outcome_label, "cohort_5_interval", cohort)
...
for (outcome_name in names(outcomes)) {
  ...
  for (cohort in cohorts2) {
    # Generate the plot
    p <- did_graph_trend_10(outcome_name, outcome_label, "cohort_10_interval", cohort)
...
* Synthetic DiD: Most cohorts show an increasing in spending, although some
suggest a decrease in spending

* TWFE by cohort consistent with initial results

* Callaway-Sant'anna: Application more robust to staggered adoption, but still 
